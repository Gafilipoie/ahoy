...
HorizontalSlider.prototype.getLeftPosition = function() {
				...
				var result =  0 - (parseInt(this.getSlideCount()/2) * this.getSlideWidth()) + leftDistance;
...
HorizontalSlider.prototype.addVideoSlide = function(index, position) {
		...
		var self = this;
		var data = this.jsonData[index].Slide;
		//var video_tag = $('<video id="videoElem'+index+'" controls class="video-js vjs-default-skin" poster="'+appURL+'img/uploads/desktop/'+data.image+'" data-setup="{}"><source src="'+appURL+'img/uploads/videos/'+data.mp4+'" type="video/mp4" /></video>');
		var video_tag = $('<video id="videoElem'+index+'" controls preload poster="'+appURL+'img/uploads/desktop/'+data.image+'"  src="'+appURL+'img/uploads/videos/'+data.mp4+'" /></video>');
		...
		//_V_("videoElem" + index + "", { "controls": true, "autoplay": false, "preload": "auto", "captions": false, "kind": false });
		html5media();
...
HorizontalSlider.prototype.loadNextImage = function(index) {
				...
				if (index <= self.getSlideCount()) {
								...
								$("<img>", { src: src }).on('load error', function() {
...
HorizontalSlider.prototype.centerSlider = function() {
				var newLeft = this.getLeftPosition();
				this.slider.stop(false,true).animate({left: newLeft});
}
...
HorizontalSlider.prototype.startAutoScroll = function (direction) {
				...
				self.autoScrollInterval = setInterval(function(){
								...
								self.isSliding = true;
								self.slider.animate({'left': newLeft}, 250, 'linear', callback);
